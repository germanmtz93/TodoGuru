"""
Task Manager Module
Handles task operations including loading, saving, adding, listing, and marking tasks as done.
"""
...
class TaskManager:
    ...
    def mark_task_done(self, task_index: int) -> None:
        ...
        if task_index < 1 or task_index > len(self.tasks):
            raise ValueError(f"Invalid task index. Please choose a number between 1 and {len(self.tasks)}")
        ...
        self._save_tasks()
        print(f"Task {task_index} marked as done: {self.tasks[array_index]['description']}")
